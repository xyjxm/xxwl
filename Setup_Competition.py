...
def setup(initialPosition = [-1.205, -0.83, 0.005], initialOrientation = [0, 0, -44.7], scenario_num = 3):
    # Try to connect to Qlabs

    # os.system('cls')
    qlabs = QuanserInteractiveLabs()
    print("Setup Competition is connecting to QLabs...")
    try:
        qlabs.open("localhost")
        print("Setup Competition connected to QLabs")
    except:
        print("Setup Competition unable to connect to QLabs")
        quit()
    ...
    x = hSystem.set_title_string('Self Driving Car Competition', waitForConfirmation=True)
    ...
    if scenario_num == 3:
        ...
        TrafficCone.spawn_degrees([2.1 + x_offset, y_offset-0.5, 0.01], [0, 0, 0], scale=[.5, .5, .5], configuration=0, waitForConfirmation=True)
